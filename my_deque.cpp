...
template <typename T>
my_deque<T>::my_deque(const my_deque& other)
    : arr_(nullptr),
      front_(other.front_),
      back_(other.back_),
      size_(other.size_),
            ...
            for (; middle_index_i < back_.i; ++middle_index_i) {
                ...
                try {
                    for (; constructed < SIZE_OF_BUCKET; ++constructed)
                        new (new_arr[middle_index_i] + constructed)
                            T(other.arr_[middle_index_i][constructed]);

                } catch (...) {
                    for (size_t j = 0; j < constructed; ++j) (new_arr[middle_index_i] + j)->~T();
                    throw;
                }
            }
            size_t counter = 0;
...
template <typename T>
my_deque<T>::my_deque(my_deque<T>&& other)
    : arr_(other.arr_),
      front_(other.front_),
      back_(other.back_),
      size_(other.size_),
...
my_deque<T>& my_deque<T>::operator=(my_deque&& other) {
    if (this != &other) {
        if (arr_) {
            if (front_.i == back_.i) {
                for (size_t i = front_.j; i <= back_.j; ++i) (arr_[front_.i] + i)->~T();
            } else {
                for (size_t i = front_.j; i < SIZE_OF_BUCKET; ++i) (arr_[front_.i] + i)->~T();
                ...
                for (size_t i = 0; i <= back_.j; ++i) (arr_[back_.i] + i)->~T();
            }
    ...
    if (++back_.j == SIZE_OF_BUCKET) {
        if (++back_.i == arr_size_) {
            ...
            try {
                ...
                if (new_arr) {
                    for (size_t i = 0; i < half; ++i) delete[] reinterpret_cast<char*>(new_arr[i]);
